#pragma once //gets rid of duplicates

#ifdef _WIN32
#  define WINDOWS_LEAN_AND_MEAN //only call for 32-bit systems
#  define NOMINMAX
#  include <windows.h>
#endif
...
//Below stuff is windows code. Makes a window
...
bool	keys[256];		// Array Used For The Keyboard Routine
    //active window or not. A flag
bool	active=TRUE;	// Window Active Flag Set To TRUE By Default
bool	fullscreen=TRUE;// Fullscreen Flag Set To Fullscreen Mode By Default

LRESULT	CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);	// Declaration For WndProc
_Scene *Scene = new _Scene(); //Global Scene
			  ...
			  LPARAM lParam)		// Additional Message Information
{
	...
	switch (uMsg)					// Check For Windows Messages
	{
		...
		{
            Scene->resizeScene(LOWORD(lParam), HIWORD(wParam)); // LoWord=Width, HiWord=Height
			return 0;			    // Jump Back
		}
	}
	...
	while(!done)					    // Loop That Runs While done=FALSE
	{
	  if (PeekMessage(&msg,NULL,0,0,PM_REMOVE))	// Is There A Message Waiting?
	  ...
	  else						        // If There Are No Messages
		{
			...
			{
			  Scene->wireFrame = !Scene->wireFrame; //change it to what wireFrame isn't (works bc it's only either true or false bc bool variable)
			}
		}
	}
